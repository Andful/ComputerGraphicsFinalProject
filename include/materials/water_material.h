...
typedef struct alignas(16) _WaterMaterialUniformData{
	glm::vec3 ks;
	float shininess;
	glm::vec3 kd;
} WaterMaterialUniformData;

class WaterMaterial : public Material {
private:
	WaterMaterialUniformData water_material_uniform;
	FragmentShader fragment_shader, xray_shader, xray_cull_shader;
	std::shared_ptr<Texture> texture, toonTexture;
public:
	WaterMaterial(glm::vec3 ks, float shininess, glm::vec3 kd, std::shared_ptr<Texture> tex, std::shared_ptr<Texture> toonTex);
